...
<footer class="footer">
    <div class="container-fluid clearfix">
            <span class="text-muted d-block text-center text-sm-left d-sm-inline-block">Copyright © 2018
              <a href="http://www.cloudware.ng">Cloudware Technologies</a>. All rights reserved.</span>
...
//        }
      ...
      function set_all_total() {
          var total_p = $(".total_per_product"), sum_total = 0;
...
//          $("#slip_modal_to_print").print();
          ...
          $.each(all_trs, function () {
              var name = $(this).find(".td_name_to_send"), quantity = $(this).find(".td_quantity_to_send"), td_total = $(this).find(".td_total_send");
//              console.log(name, quantity, td_total);
          ...
          var today = new Date();
          var date_to_send = today.getFullYear() + "-" + today.getMonth() + "-" + today.getDay(), payment_method = 1;
          ...
          $.ajax({
              ...
              type: "post",
              data: {values: JSON.stringify(data),date_to_send: date_to_send, payment_method: payment_method,randStr: randStr},
              success: function(data){
                  console.log(data);
                  var json_data = JSON.parse(data);
                  if(json_data.status == 1){
                      var alert_message = $("<div class='success alert-success'>Transaction Completed Successfully, Redirecting you very soon</div>");
                  }else if(json_data.status == -1){
                      var alert_message = $("<div class='success alert-warning'>Transaction Partially Successfully, Redirecting you very soon</div>");
                  }else{
                      var alert_message = $("<div class='success alert-danger'>Transaction Completed Successfully, Redirecting you very soon</div>");
                  }
                  $(".modal").hide();
      ...
      $("#print_slip_btn").click(function () {
          var all_trashes = $(".trash_created"), new_tbody = $("<tbody></tbody>");
          if($("#receipt_table").length > 0){
              $("#receipt_table").find("tbody").empty();
          }

          var total_price_holder = $("#total_price_holder").text().toString().split(" "), customer_change = $("#customer_change").text().toString().split(" ");

          var $tp_td = "<td>Total Price: </td>", tp_val = "<td>" + total_price_holder[total_price_holder.length - 1] +  "</td>";
          var cc_td = "<td>Change: </td>", cc_val = "<td>" + customer_change[customer_change.length - 1] + "</td>";

          var amp_td = $("<td>Amount Paid: </td>"), amp_val = $("<td>" + $('#amount_collected').val() + "</td>");

          console.log(customer_change);

          var last_tr = $("<tr></tr>").append(amp_td).append(amp_val).append($tp_td).append(tp_val);
          var d_last_tr = $("<tr></tr>").append("<td></td>").append(cc_td).append(cc_val).append("<td></td>");

          $.each(all_trashes, function () {
              var pro_name = $(this).find(".pro_name"), serial_number = $(this).find('.serial_number'),
                  pro_quantity = $(this).find(".pro_quantity").find("input"), pro_total = $(this).find(".pro_total");

              var new_tr = $("<tr></tr>");
              var name_td = $("<td class='td_name_to_send'>" + pro_name.text() + "</td>"), quantity_td = $("<td class='td_quantity_to_send'>" + pro_quantity.val() + "</td>"),
                  total_td = $("<td class='td_total_send'>" + pro_total.text() + "</td>"),sn_td = $("<td>" + serial_number.text() + "</td>");
        ...
        {
            var id_html = $("#" + id).html(), body_html = $("body").html();
            $("body").html(id_html);
            window.print();
            $("body").html(body_html);
        }
    ...
    $(document).on("keypress", function (e) {
        if(e.keyCode == 13){
            var div_text = $("#customer_change").text(), str_to_check = "Customer";
...
//        console.log($("#print_slip_btn"));
      $("#amount_collected").on("keyup", function () {
          ...
          var total_goods_text = $("#total_price_holder").text(), total_goods_amount = total_goods_text.split(":"), tot_goods_amt = Number(total_goods_amount[1]);

          if(isNaN(amount_collected)){
              $("#error_text").css({"color":"red","transition":"1s ease-in-out"}).text("Please, type a number");
              ...
              if(amount_collected >= tot_goods_amt){
                  ...
                  $("#customer_change").html("Customer's change is: <span style='font-weight: bold;font-size: 20px;'>" + customer_change + "</span>").css("transition","1s ease-in-out");
                  $("#error_text").text("");
                  $("#print_slip_btn").css("pointer-events","all");
              }else{
                  $("#error_text").css({"color":"red","transition":"1s ease-in-out"}).text("Sorry, amount collected can't be lesser than total amount of goods sold");
...
//            }
            ...
            $.ajax({
                ...
                success:function (data) {
                    var json_str = JSON.parse(data), len = json_str.length;
                    if(len > 0){
                        for(json_key in json_str){
                            if(json_str[json_key].stock_count < 1){
                                console.log($(".message_ctn"));
                                $(".message_ctn").text("Sorry, " + $product_name + " is no longer in stock").css({'color':'red'});
//                                return false;
                            }else{
                                $(".message_ctn").text("");
                                var tr_pop = $("<tr class='trash_created' row_to_delete='" + Number($(".trash_created").length) + "'></tr>"), td_array = [];

                                td_array.push($("<td class='serial_number'>" + (Number($(".trash_created").length) + 1) + "</td>"));
                                td_array.push($("<td class='pro_name'>" + json_str[json_key].name + "</td>"));
                                var btns = generate_btns("qty" + Number($(".trash_created").length) + 1, json_str[json_key].stock_count);
                                td_array.push($("<td class='pro_quantity'>" + btns + "</td>"));
                                td_array.push($("<td class='pro_price price_" + "qty" + Number($(".trash_created").length) + 1 + "'>" + json_str[json_key].price + "</td>"));
                                td_array.push($("<td class='pro_total total_per_product total_" + "qty" + Number($(".trash_created").length) + 1 + "'>" + json_str[json_key].price + "</td>"));
                                td_array.push($("<td class='delete_row' item_to_delete='" + Number($(".trash_created").length) + "'><button type='button' data-tooltip='Remove' class='btn btn-icons btn-rounded btn-danger'><i class='mdi mdi-delete'></i></button></td>"));
...
//                    console.log(json_str[0].name);
	    ...
	    $(".frequently_bought").click(function () {
            ...
            td_array.push($("<td class='pro_quantity'>" + btns + "</td>"));
            td_array.push($("<td class='pro_price price_" + "qty" + Number($(".trash_created").length) + 1 + "'>" + json_str[2] + "</td>"));
            td_array.push($("<td class='pro_total total_per_product total_" + "qty" + Number($(".trash_created").length) + 1 + "'>" + json_str[2] + "</td>"));
            td_array.push($("<td class='delete_row' item_to_delete='" + Number($(".trash_created").length) + "'><button type='button' data-tooltip='Remove' class='btn btn-icons btn-rounded btn-danger'><i class='mdi mdi-delete'></i></button></td>"));
	...
	function increase(id) {
        var $input_val = Number($("#" + id).val()),value_to_set = 0, stock_count = $("#" + id).attr("stock_count");
        // alert($input_val);
        if($input_val == 0){
            value_to_set = 1;
        }else if($input_val > 0){
...
//        set_modal_cart_amount();
	...
	function generate_btns(id, stock_count) {
        var btn = '<button class="btn btn-danger btn-sm btn-rounded btn_to_calc btn_decrease" type="button" data_to_use="' + id + '">-</button> ' +
        '<input size="1" type="text" stock_count="' + stock_count + '" value="1" id="' + id + '" name="item_quantity" readonly="readonly">' +
